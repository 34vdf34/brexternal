...
grep -q "^dtparam=audio=off$" $CFG_PATH || echo "dtparam=audio=off" >> $CFG_PATH
grep -q "^dtparam=i2c_arm=on$" $CFG_PATH || echo "dtparam=i2c_arm=on" >> $CFG_PATH
grep -q "^dtoverlay=vc4-fkms-v3d,noaudio$" $CFG_PATH || echo "dtoverlay=vc4-fkms-v3d,noaudio" >> $CFG_PATH
...
grep -q "^dtoverlay=vc4-kms-v3d-pi4,noaudio$" $CFG_PATH || echo "dtoverlay=vc4-kms-v3d-pi4,noaudio" >> $CFG_PATH
...
# USB boot:
grep -q "^/dev/sda2          /boot           vfat    defaults        0       0$" $FSTAB_PATH \
        || echo "/dev/sda2          /boot           vfat    defaults        0       0" >> \
        $FSTAB_PATH
